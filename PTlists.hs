...
allIntegersInclusive a b = if b < a then reverse [b .. a] else [a .. b]
...
allIntegersExcludingFirst a b = if b < a then tail (reverse [b .. a]) else tail [a .. b]
...
divisors n = [ x | x <- init [1..n], n `mod` x == 0]  
...
allPerfects n = [ x | x <- [1..n], isPerfect x ==True]
...
isRigid n = sum ([(read :: String -> Int)[x] | x <- show n]) == product ([(read :: String -> Int)[x] | x <- show n])
...
allRigids n =[ x | x <- [1..n], isRigid x ==True]
...
fizzBuzz n f b = [ if x `mod` (f*b) == 0 then "FizzBuzz" else if  x `mod` f == 0 then "Fizz" else if x `mod` b == 0 then "Buzz" else show x | x <- [1..n]]
