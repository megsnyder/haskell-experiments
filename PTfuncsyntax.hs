...
windChillRisk t v
  | twc < (-64) = "Extreme Risk"
  | twc < (-53) = "Severe Risk"
  | twc < (-38) = "Very High Risk"
  | twc < (-17) = "High Risk"
  | twc < 16 = "Moderate Risk"
  | otherwise = "Low Risk"
...
interestTable p r n = [round p] ++ interestTable (p*(1+0.01*r)) r (n-1)  -- your function here (may need more patterns!)
...
numListToNum [] = 0
numListToNum (x:nums) = x * 10 ^ length nums + numListToNum nums -- put your code here (more patterns needed?)
...
wordsToPhone w =  numListToNum [ charToPhoneDigit x | x <- w, x `elem` ['a'..'z'] ++ ['A'..'Z']] -- put your code here